...
namespace RbTree
{
    ...
    public class RbTree<KeyType, ValueType> : ITree<KeyType, ValueType> where KeyType : IComparable<KeyType>
    {
        public class Node
        {
            public Node Left;
            ...
            public List<ValueType> Values { get; private set; }
            //public ValueType Value { get; private set; }
            public override string ToString() => _isLeaf ? "Nil" : $"({Key.ToString()}: {Values.Count}, {Color}, {Values.ToString()})";
        ...
        private Node Add(KeyType key, ValueType value)
        {
            ...
            if (duplicate != Nil)
            {
                //duplicate.Values.AddLast(value);
                duplicate.Values.Add(value);
                return duplicate;
            }
        ...
        public void Remove(KeyType key, ValueType value)
        {
            ...
            if (del == Nil)
                return;
            if (del.Values.Count > 1)
            {
                del.Values.Remove(value);
                return;
            }
            Delete(del);
        }
        ...
        public List<ValueType> GetRange(KeyType lower, KeyType upper)
        {
            List<ValueType> values = new List<ValueType>();
            GetRangeSubtree(Root, lower, upper, values);
            return values;
        }

        public List<ValueType> GetRange(KeyType lower, KeyType upper, Predicate<ValueType> predicate, Func<ValueType, bool, ValueType> function)
        {
            List<ValueType> values = new List<ValueType>();
            GetRangeSubtree(Root, lower, upper, values, predicate, function);
            return values;
        }
        ...
        public void GetRangeSubtree(Node node, KeyType lower, KeyType upper, List<ValueType> list)
        {
            int direction = directionFromBounds(lower, upper, node.Key);
            ...
            //if(direction == 0) list.Add(node.Values.First.Value);
            if(direction == 0) list.AddRange(node.Values);
            if(direction >= 0 && node.Right != null) GetRangeSubtree(node.Left, lower, upper, list);
            if(direction <= 0 && node.Left != null) GetRangeSubtree(node.Right, lower, upper, list);
        }

        public void GetRangeSubtree(Node node, KeyType lower, KeyType upper, List<ValueType> list, Predicate<ValueType> predicate, Func<ValueType,bool,ValueType> function)
        {
            int direction = directionFromBounds(lower, upper, node.Key);

            if (direction == 0)
            {
                int valuesListLength = node.Values.Count;
                for(int i = 0; i < valuesListLength; i++)
                {
                    //var listNode = GetNodeAtIndex(node.Values, i);
                    if (predicate != null && !predicate(node.Values[i])) continue;
                    //if (predicate != null && !predicate(listNode.Value)) continue;                    
            ...
            if(function != null)
            {
                ...
                {
                    //var listNode = GetNodeAtIndex(node.Values, i);
                    //listNode.Value = function(listNode.Value, direction == 0 && predicate(listNode.Value));
                    node.Values[i] = function(node.Values[i], direction == 0 && predicate(node.Values[i]));
                }
            }
        }
        ...
        public void Print()
        {
            ...
            foreach (var n in list)
            {
                ...
                Write($"{n.node.Key}: {n.node.Values.Count}");
                ResetColor();
                WriteLine();
            }
        }
        ...
        {
            return GetRange(leftBound, rightBound);
        }

        void ITree<KeyType, ValueType>.AddElement(KeyType key, ValueType value)
        {
            Add(key, value);
        }

        public void RemoveValue(KeyType key, ValueType node)
        {
            throw new NotImplementedException();
        }
        ...
        public IEnumerator<ITree<KeyType, ValueType>> GetEnumerator()
        {
            throw new NotImplementedException();
        }
        ...
        {
            return GetRange(leftBound, rightBound, predicate, function);
        }
    }
